...
import { auth, firestore, provider, rtdb, storage } from "../lib/firebase";
...
export function useEverDocsWorkspace() {
  ...
  const timersRef = useRef({ save: null, typing: null });
  ...
  async function updatePresence(activeDoc = null) {
    const user = stateRef.current.currentUser;
    ...
    listenersRef.current.presence = onValue(
      ...
      (snapshot) => {
        ...
        const active = Object.entries(values)
          .filter(([, value]) => value && value.online === true && value.activeDoc === docId)
          .map(([uid]) => uid);
  ...
  function setActiveChannel(channelId, manualSelection = false) {
    ...
    listenersRef.current.channels = onSnapshot(
      ...
      (snapshot) => {
        ...
        if (
          ...
          !items.some((item) => item.id === stateRef.current.currentChannelId)
        ) {
          stateRef.current.currentChannelId = "";
          setCurrentChannelId("");
        }
  ...
  async function openDocument(docId) {
    ...
    listenersRef.current.doc = onSnapshot(
      ...
      (snapshot) => {
        ...
        if (
          !stateRef.current.hasManualChannelSelection ||
          !stateRef.current.currentChannelId ||
          docChannelChanged
        ) {
  ...
  async function saveCurrentDocument(trigger = "manual") {
    const docId = stateRef.current.currentDocId;
    if (!docId) {
  ...
  function monitorRealtimeConnection() {
    ...
    listenersRef.current.connection = onValue(ref(rtdb, ".info/connected"), (snapshot) => {
      ...
      if (stateRef.current.realtimeConnected) {
        const suffix = stateRef.current.currentDocId
          ? ` in ${stateRef.current.currentDocId.slice(0, 6)}...`
          : "";
  ...
  function handleEditorInput(nextValue) {
  ...
  async function inviteToChannel() {
    ...
    const targetUid = inviteChannelUid.trim();
    if (!targetUid) {
  ...
  async function uploadFile(file) {
    ...
    try {
      ...
      const uploadRef = storageRef(storage, prep.storagePath);
  ...
  async function requestDownloadUrl(fileId) {
    try {
